...
static int stm32mp1_clktree(struct udevice *dev)
{
	 ...
	 */
	if (priv->osc_clk[_LSI])
		stm32mp1_lsi_set(rcc, 1);

	if (priv->osc_clk[_LSE]) {
		int bypass, digbyp;
		u32 lsedrv;
...
static void stm32mp1_osc_clk_init(struct udevice *dev, const char *name,
				  struct stm32mp1_clk_priv *priv, int index)
{
	struct clk *clk = devm_clk_get(dev, name);

	if (!IS_ERR(clk))
		priv->osc_clk[index] = clk;
	else
		pr_warn("No clock \"%s\"", name);
}
...
static void stm32mp1_osc_init(struct udevice *dev)
{
	...
	for (i = 0; i < NB_OSC; i++) {
		stm32mp1_osc_clk_init(dev, name[i], priv, i);
		debug("Clock %d: %s , %luHz\n",
		      i, name[i], clk_get_rate(priv->osc_clk[i]));
	}
}
